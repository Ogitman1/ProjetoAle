...
function generateEquation(){//Passo 1: Lógica pra montar o "problema"
    const x = Math.floor(Math.random() *(max - min+ 1) ) + min;
    const equation = `What is X + ${x} = ${x *2} ?`
    return {x, equation};
}
function displayEquation() {//Passo 2: Pegando o document pra mostrar o problema
  ...
  function checkanswer(){
    const answer = parseInt(document.getElementById("answer").value);//transformar o valor do input em int
